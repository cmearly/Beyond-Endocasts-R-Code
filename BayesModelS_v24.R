...
predictiveUnknown = function(filename, treeDataAll, missingList, lambdaUpperBound, kappaUpperBound, path = default){
  ...
  index <- c()
  for(k in 1:length(missingListOrigin)){
    index<-c(index,which(colnames(predictiveSample) == missingListOrigin[k]))  
  }
  predictiveSampleFinal <- as.matrix(predictiveSample[, index])
  colnames(predictiveSampleFinal) <- colnames(predictiveSample)[index]
...
predictiveSummary = function(predictiveSampleWrapper, missingList, path){
  ...
  for(i in 1:n){
    ...
    hist(predictiveSample[, index], main = missingList[i], xlab = "predictive samples", col = "blue")
